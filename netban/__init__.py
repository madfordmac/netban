...
class NetBanManager(object):
	...
	async def setup(self):
		...
		p = await asyncio.create_subprocess_exec(['/usr/sbin/iptables','-I','INPUT',insert_at,'-m','set','--set','netbanlocal','src','-j','DROP'])
		r = await p.wait()
		assert r == 0, "Inserting netbanlocal rule failed."
		p = await asyncio.create_subprocess_exec(['/usr/sbin/iptables','-I','INPUT',insert_at,'-m','set','--set','netbannet','src','-j','DROP'])
		r = await p.wait()
